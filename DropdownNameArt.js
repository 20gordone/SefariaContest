...
function nameSearch(){
    ...
    document.getElementById("vary").innerHTML ="<label for=\"versesdropdown\">Pasuk results:</label><select id=\"versesdropdown\"></select>"
...
function formatArt(sources){
    ...
    var artPerek = jQuery.getJSON( "https://www.sefaria.org/api/texts/" + book + "." + chapter, function(data) {
        ...
        console.log(artPasuk.charAt(letInds[0]))
...
function fA2(sources){
    //sources = sources.substring(sources.length)
    console.log("blamo " + String(sources))
    var source = sources.split(".");
    var lookup = source[0];
    var book = source[1];
    var chapter = source[2];
